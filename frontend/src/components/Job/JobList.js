import React from 'react';
import { makeStyles } from '@material-ui/core/styles';
import Table from '@material-ui/core/Table';
import TableBody from '@material-ui/core/TableBody';
import TableCell from '@material-ui/core/TableCell';
import TableHead from '@material-ui/core/TableHead';
import Button from '@material-ui/core/Button';
import TableRow from '@material-ui/core/TableRow';
import { Link } from "react-router-dom";
import Title from '../Common/Title';
import fetch from 'node-fetch';

const useStyles = makeStyles((theme) => ({
  seeMore: {
    marginTop: theme.spacing(3),
  },
}));

export default function JobInstance() {
  const [rows, setRows] = React.useState([]);
  const [skip, setSkip] = React.useState(0);
  const [isMore, setIsMore] = React.useState(true);
  const take = 30;

  const callList = async () => {
    const res = await fetch(`${process.env.REACT_APP_API_SERVER}/batch/jobInstances?take=${take}&skip=${skip}`);
    const data = await res.json();
    setRows(r => [...r, ...data.batchJobsInstance]);
    if (data.batchJobsInstance.length < take) setIsMore(false);
  }
  React.useEffect(() => {
    callList();
  }, [skip]);
  
  const classes = useStyles();
  return (
    <React.Fragment>
      <Title>Job Instance</Title>
      <Table size="small">
        <TableHead>
          <TableRow>
            <TableCell>Job Id</TableCell>
            <TableCell>Version</TableCell>
            <TableCell>Job Name</TableCell>
            <TableCell>Job Key</TableCell>
          </TableRow>
        </TableHead>
        <TableBody>
          {rows && rows.map((row) => (
            <TableRow key={row.jobInstanceId}>
              <TableCell><Link to={`/jobExecution/${row.jobInstanceId}`}><Button>{row.jobInstanceId}</Button></Link></TableCell>
              <TableCell>{row.version}</TableCell>
              <TableCell>{row.jobName}</TableCell>
              <TableCell>{row.jobKey}</TableCell>
            </TableRow>
          ))}
        </TableBody>
      </Table>
      {isMore && <div className={classes.seeMore}>
        <Button onClick={e => { setSkip(s => s + take); }}>
          See more orders
        </Button>
      </div>}
    </React.Fragment>
  );
}